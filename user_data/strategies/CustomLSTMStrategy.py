...
class CustomLSTMStrategy(IStrategy):
    """
    ...
    def feature_engineering_expand_all(self, dataframe: DataFrame, period: int,
        ...
        dataframe['%-fast-k-period'], dataframe['%-fast-d-period'] = stoch_fast["fastk"], stoch_fast["fastd"]
        dataframe['%-fast-j-period'] = list(map(lambda x, y: 3 * x - 2 * y, dataframe['%-fast-k-period'], dataframe['%-fast-d-period']))
        dataframe['%-cti-preid'] = pta.cti(dataframe["close"], length=20)
