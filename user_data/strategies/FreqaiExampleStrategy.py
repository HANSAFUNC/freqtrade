...
class FreqaiExampleStrategy(IStrategy):
    ...
    def feature_engineering_expand_all(
        ...
        *Only functional with FreqAI enabled strategies*
        ...
        """
        # for tf in self.include_timeframes:
        #     print(tf)

        dataframe["%-adx-period"] = ta.ADX(dataframe["high"],dataframe["low"],dataframe["close"],timeperiod=14)
        ...
        diff, dea, hist = ta.MACD(dataframe["close"], 12, 26, 9)
        stick = hist * 2
        dataframe["%-diff-period"] = diff
        dataframe["%-dea-period"] = dea
        dataframe["%-stick-period"] = stick

        slowk, slowd = ta.STOCH(dataframe["high"], dataframe["low"], dataframe["close"], 9, 3,
                                   1, 3, 1)

        # 求出J值，J = (3 * D) - (2 * K)
        slowj = list(map(lambda x, y: 3 * x - 2 * y, slowk, slowd))
        dataframe["%-k-period"] = slowk
        dataframe["%-d-period"] = slowd
        dataframe["%-j-period"] = slowj
        ...
        # 最后统一前向／后向填充，并把残余 NaN 置 0
        dataframe.ffill(inplace=True)  # 用上一个非空值填充
        dataframe.bfill(inplace=True)  # 用下一个非空值填充
        dataframe.fillna(0, inplace=True)  # 任何还剩的 NaN = 0
        print("生成的列名：", dataframe.columns.tolist())
        return dataframe
